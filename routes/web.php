...
//routes/web.php 文件用于定义 web 界面的路由。这里面的路由都会被分配给 web 中间件组，
//它提供了会话状态和 CSRF 保护等功能。定义在 routes/api.php 中的路由都是无状态的，并且被分配了 api 中间件组。
//大多数的应用构建，都是以在 routes/web.php 文件定义路由开始的。
//可以通过在浏览器中输入定义的路由 URL 来访问 routes/web.php 中定义的路由。例如，你可以在浏览器中输入 http://your-app.dev/user 来访问以下路由
//routes/api.php 文件中定义的路由通过 RouteServiceProvider 被嵌套到一个路由组里面。
//在这个路由组中，会自动添加 URL 前缀 /api 到此文件中的每个路由，这样你就无需再手动添加了。
...
//});
//CSRF 保护
//指向 web 路由文件中定义的 POST、PUT 或 DELETE 路由的任何 HTML 表单都应该包含一个 CSRF 令牌字段，
//否则，这个请求将会被拒绝。可以在 CSRF 文档 中阅读有关 CSRF 保护的更多信息：
...
//这个方法可以快速地实现重定向，而不再需要去定义完整的路由或者控制器。
...
//想知道所有可访问的方法，可以查看 API 文档，了解 Route facade 和 Route 实例 的基础类。
